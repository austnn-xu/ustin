...
(function (root, factory) {
  ...
  /**
   * Below this summed score, do not name an object — narrow by material
   * instead. Measured on held-out TrashNet, answers below this line are close
   * to a coin flip, and a confident wrong answer is the thing that makes the
   * whole app feel broken.
   */
  const RECOGNITION_FLOOR = 0.6;
  ...
  /**
   * The head is never allowed to VETO a candidate, only to lift one, and this
   * is deliberate against the benchmark. On held-out TrashNet a veto scores
   * better — but every TrashNet image is one of the six materials it knows,
   * and most of what people actually photograph is not. It has no "none of
   * the above" to reach for, so off its distribution it is confidently wrong:
   * a cat reads 58% plastic, a rocket 82% cardboard, a ceramic espresso cup
   * 91% metal. A veto on readings like those deletes correct answers, and the
   * benchmark cannot show it happening.
   */
  ...
  /**
   * ILSVRC is ordered: 0-397 are animals, 398-969 artifacts, 970-980 natural
   * scenes, 981-983 people, 984+ plants and food. A photograph the object
   * model reads as a living thing or a landscape is not a household item, and
   * the material head — which has no "none of the above" — would otherwise
   * answer anyway. A cat reads 58% plastic.
   */
  const NOT_WASTE_RANGES = [[0, 397], [970, 983]];

  /** Above this, a living-thing or landscape reading is taken at its word. */
  const NOT_WASTE_FLOOR = 0.25;

  function looksLikeWaste(index, probability) {
    if (probability < NOT_WASTE_FLOOR) return true;
    return !NOT_WASTE_RANGES.some(([lo, hi]) => index >= lo && index <= hi);
  }
  ...
  function interpret(probabilities, options) {
    ...
    const top = material ? topMaterial(material) : null;
    ...
    return {
      ...
      // The full ranking, for the narrowed picker: an object that failed to
      // clear the floor is still better evidence than alphabetical order.
      ranked: candidates,
      recognized: Boolean(best) && best.score >= RECOGNITION_FLOOR,
      saw: { label: shortLabel(LABELS[topIndex]), probability: topProbability },
      material: top,
      // A cat is not a recycling question, and neither model should pretend.
      looksLikeWaste: looksLikeWaste(topIndex, topProbability),
    };
  }
   ...
   */
  function byMaterial(material, options) {
    const top = topMaterial(material);
    if (!top || !top.family || top.probability < MATERIAL_FLOOR) return null;

    const ranked = (options && options.ranked) || [];
    const rank = new Map(ranked.map((c, i) => [c.object.id, i]));

    const objects = rules.OBJECTS
      .filter((o) => (FAMILIES.get(o.id) || new Set()).has(top.family))
      .sort((a, b) => {
        // Objects the classifier scored at all come first, in its order. It
        // was not confident enough to name one, which is not the same as
        // having no opinion.
    ...
    return { material: top, objects: limit ? objects.slice(0, limit) : objects };
  }
